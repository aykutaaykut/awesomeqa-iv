...
class TicketRepository:
    ...
    def resolve_ticket(
        ...
        ticket.ts_last_status_change = datetime.now(timezone.utc).isoformat()
    ...
    def get_messages(
        self, skip: Optional[int] = 0, limit: Optional[int] = None
    ) -> list[MessageModel]:
        """
        Get at most `limit` messages after skipping `skip` messages.
    ...
    def get_ticket_message(self, ticket_id: str) -> MessageModel:
        """
        Gets the ticket message with the given `ticket_id`.
        ...
        -------
        MessageModel
            Message belonging to the ticket with the given `ticket_id`.
